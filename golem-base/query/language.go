...
func (e *LessThan) Evaluate(b *QueryBuilder) string {
	...
	b.tableBuilder.WriteString(" AS (")
	b.tableBuilder.WriteString(
		"SELECT entity_key FROM annotations WHERE annotation_key = ? AND numeric_value < ?",
	)
	b.tableBuilder.WriteString(")")
...
func (e *Ownership) Evaluate(b *QueryBuilder) string {
	...
	b.tableBuilder.WriteString(" AS (")
	b.tableBuilder.WriteString(
		"SELECT entity_key FROM annotations WHERE annotation_key = \"$owner\" AND string_value = ?",
	)
	b.tableBuilder.WriteString(")")
...
func (e *Equality) Evaluate(b *QueryBuilder) string {
	...
	if e.Value.String != nil {
		b.tableBuilder.WriteString(
			"SELECT entity_key FROM annotations WHERE annotation_key = ? AND string_value = ?",
		)
