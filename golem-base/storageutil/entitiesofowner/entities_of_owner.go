...
func AddEntity(db StateAccess, owner common.Address, entity common.Hash) (err error) {
	defer func() {
		if err != nil {
			log.Error("Error adding entity", "owner", owner, "entity", entity, "error", err)
		}
	}()
	log.Info("Adding entity", "owner", owner, "entity", entity)
	ownerKey := crypto.Keccak256Hash(OwnerEntitiesSalt, owner.Bytes())
	return keyset.AddValue(db, ownerKey, entity)
}

func RemoveEntity(db StateAccess, owner common.Address, entity common.Hash) (err error) {
	defer func() {
		if err != nil {
			log.Error("Error removing entity", "owner", owner, "entity", entity, "error", err)
		}
	}()
	log.Info("Removing entity", "owner", owner, "entity", entity)
	ownerKey := crypto.Keccak256Hash(OwnerEntitiesSalt, owner.Bytes())
