...
func SyncSQLiteFromChain(blockNumber uint64, blockHash common.Hash, blockRoot common.Hash, sqlStore *SQLStore, fuseDriver *fuse.FuseDriver, chainID *big.Int, db state.Database) error {
	...
	log.Info("Hasher after sync", hex.EncodeToString(hasherRoot))

	sqlStore.hasher.Build()
	hasherRoot = sqlStore.hasher.Root()
	log.Info("Hasher after synd and build", hex.EncodeToString(hasherRoot))
