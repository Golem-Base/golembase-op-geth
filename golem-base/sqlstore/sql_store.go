...
const entitiesSchemaVersion = uint64(8)
...
func NewStore(dbFile string, historicBlocksCount uint64) (*SQLStore, error) {
	...
	db, err := sql.Open("sqlite3", fmt.Sprintf("file:%s?mode=rwc&_synchronous=full&_busy_timeout=11000&_journal_mode=WAL&_auto_vacuum=incremental&_foreign_keys=true&_txlock=immediate", dbFile))
	if err != nil {
	...
	readDB, err := sql.Open("sqlite3", fmt.Sprintf("file:%s?&_synchronous=full&_query_only=true&_busy_timeout=11000&_journal_mode=WAL&_foreign_keys=true&_txlock=deferred", dbFile))
	if err != nil {
		return nil, fmt.Errorf("failed to open database: %w", err)
	}
